...
double **past_idle; // for menu governor
...
int main(int argc, char **argv){
	...
	if(argc<4) {
		printf("use: [freq] [load] [core count]\n");
		return 0;
	}
	...
	double package_wakeup_latency=500;
				...
				if(server[assigned_server].time_vacation_end==-1){
					if(package_sleep >0 && package.time_arrived>0){//package sleep enabled and package idle
						if(time-server[assigned_server].time_arrived>=(C_state_wakeup_latency[server_c_state]-C_state_exit_latency[server_c_state])){
							...
							server[assigned_server].time_vacation_end=server[assigned_server].time_arrived+(C_state_wakeup_latency[server_c_state]-C_state_exit_latency[server_c_state])+package_wakeup_latency;
						}
	...
	if(!quiet){
		...
		printf("average core power %f\n",(Pa*overall_busy_ratio+Pmax*overall_wakeup_ratio+overall_idle_power*overall_idle_ratio)); 
	...
	printf("%d\t%.2f\t%f\t%f\t%f\t%d\t%d\n",m,p,(Pa*overall_busy_ratio+Pmax*overall_wakeup_ratio+overall_idle_power*overall_idle_ratio)*server_count,S*(1-overall_package_idle/time),overall_latency/pkt_processed,nfp,nnp);
	// for (j=0;j<latency_bound+1;j++)
		// hist_array[j]=0;
