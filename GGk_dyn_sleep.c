...
int main(int argc, char **argv){
	...
	while (pkt_index < PKT_limit){
		...
		if (next_event == event[0]){ // *** Event #1 (arrival)
			...
			if(server[assigned_server].state==0){ //core idle
				...
				double slack=LC-(queued_pkt[assigned_server])*Ts_nf_dvfs*1e6/2+accu_arrival_time[assigned_server]/queued_pkt[assigned_server];
	...
	for(j=0;j<package_idle_counter;j++){
		overall_package_idle=overall_package_idle+package_idle_time[j];
	}
	...
	printf("%d\t%.2f\t%f\t%f\t%d\t%d\t%d\tC%d\n",m,p,(Pa*(overall_busy_ratio+overall_wakeup_ratio)+Pc*overall_idle_ratio)*server_count,overall_latency/pkt_processed,nfp,nnp,LC,C_state);
	// for (j=0;j<latency_bound+1;j++)
		// hist_array[j]=0;
